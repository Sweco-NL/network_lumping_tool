...
from src.network_lumping.preprocessing.general import (
    remove_z_dims,
...
for layer in hydro_objects_layers:
    if layer in fiona.listlayers(hydro_objects_gpkg):
        hydro_objects = pd.concat(
            [hydro_objects, gpd.read_file(hydro_objects_gpkg, layer=layer)]
        )
...
hydro_objects[hydro_objects.preprocessing_split == "split"].plot(
    ax=ax, color="red", zorder=1
)
