...
outflow_point_gpkg = Path(base_dir, "0_basisdata.gpkg")
outflow_points = gpd.read_file(outflow_point_gpkg, layer="test_outflow_points")
...
outflow_points['nearest_node'] = outflow_points.geometry.apply(lambda point: find_nearest_node(point, nodes=nodes))

# select random nodes
# no_nodes = 2
# nodes_selection = np.array(random.sample(range(0, len(nodes)), no_nodes))
nodes_selection = np.array(outflow_points['nearest_node'].tolist())
no_nodes = len(nodes_selection)
